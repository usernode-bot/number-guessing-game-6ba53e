...
function handleExplorerProxy(req, res, pathname, opts) {
  ...
  const host = upstream.replace(/^https?:\/\//, '');

  let body = '';
  const clientReq = (isHttps ? https : http).request(
    { host, path: upstreamPath + (req.url.includes('?') ? req.url.slice(req.url.indexOf('?')) : ''), method: req.method, headers: { accept: 'application/json', 'user-agent': 'usernode-dapp-proxy/1' } },
    (proxyRes) => {
  ...
  clientReq.on('error', (e) => {
    ...
    if (!res.headersSent) res.status(502).json({ error: 'explorer unavailable', detail: e.message });
  });
...
function createMockApi(opts) {
  opts = opts || {};
  const mockTxs = opts.transactions || [];
  ...
  function handleRequest(req, res, pathname) {
    ...
    if (pathname === '/__mock/getTransactions') {
      const body = req.body || {};
      const field = body.queryField || 'recipient';
      const pubkey = body.pubkey || appPubkey;
      const filtered = mockTxs.filter((t) => {
        if (field === 'recipient') return (t.to || t.destination_pubkey) === pubkey;
        if (field === 'sender') return (t.from || t.from_pubkey) === pubkey;
        return true;
      });
    ...
    if (pathname === '/__mock/sendTransaction') {
      ...
      const tx = {
        ...
        from_pubkey: body.from || appPubkey,
        to: body.to,
        destination_pubkey: body.to,
        amount: body.amount,
        memo: typeof body.memo === 'string' ? body.memo : JSON.stringify(body.memo),
  ...
  return { transactions: mockTxs, handleRequest };
}
...
async function discoverChainInfo(opts) {
  ...
  if (localDev) return { chainId: 'mock', genesisTimestampMs: Date.now() - 86400000 };
...
async function fetchAllTransactions(opts) {
  ...
  if (localDev && mockTransactions) {
    return mockTransactions.filter((t) => {
      if (queryField === 'recipient') return (t.to || t.destination_pubkey) === pubkey;
      if (queryField === 'sender') return (t.from || t.from_pubkey) === pubkey;
      return true;
    });
  }
...
function createAppStateCache(opts) {
  ...
  const useNodeStream = opts.useNodeStream !== false && !!nodeRpcUrl && !localDev;
  ...
  async function start() {
    try {
      const info = await discoverChainInfo({ nodeRpcUrl, localDev });
  ...
  return { start, handleRequest, isStreamReady, getRawTransactions, getStats, appPubkey, name };
}
...
function createNodeStatusProbe(opts) {
  ...
  async function tick() {
    if (localDev) {
      snapshot = { status: 'mock', streams: {}, explorerHasBeenOk: true };
      for (const [k, fn] of Object.entries(streams)) snapshot.streams[k] = fn();
      return;
    }
